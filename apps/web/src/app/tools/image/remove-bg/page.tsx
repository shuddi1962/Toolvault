...
import { ArrowLeft, Download, Loader2, Eraser } from "lucide-react";
  ...
  const handleRemove = async () => {
    ...
    try {
      const response = await fetch("/api/image/remove-bg", {
        ...
        body: JSON.stringify({ fileKey: `uploads/${file.name}` }),
      });
      const data = await response.json();
      if (data.success) setResult(data.data.url);
      else setError(data.error || "Failed to remove background");
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Eraser className="h-8 w-8 text-pink-600" />
            <h1 className="text-3xl font-bold text-gray-900">Remove Background</h1>
          </div>
          <p className="text-gray-600">Remove background from images automatically using AI.</p>
        </div>
        <Card>
          <CardContent className="p-6 space-y-6">
            ...
            {result && (
              <div className="p-4 bg-green-50 rounded-lg">
                <p className="text-green-700 mb-3">Background removed!</p>
                <Button asChild><a href={result} download><Download className="h-4 w-4 mr-2" /> Download PNG</a></Button>
              </div>
            )}
            <Button onClick={handleRemove} disabled={!file || loading} className="w-full">
              {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Processing...</> : <><Eraser className="h-4 w-4 mr-2" /> Remove Background</>}
            </Button>
          </CardContent>
        </Card>
      </div>
