  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch("/api/convert/word-to-pdf", {
        ...
        body: JSON.stringify({ fileKey: `uploads/${file.name}`, from: "docx", to: "pdf" }),
      });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Convert Word documents (.doc, .docx) to PDF format.</p>
        </div>
        <Card>
          <CardContent className="p-6 space-y-6">
            <FileDropzone onFilesSelect={(f) => setFile(f[0] || null)} accept={[".doc", ".docx"]} multiple={false} loading={loading} />
            ...
            <Button onClick={handleConvert} disabled={!file || loading} className="w-full">
              {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Converting...</> : <><FileText className="h-4 w-4 mr-2" /> Convert to PDF</>}
            </Button>
          </CardContent>
        </Card>
      </div>
