  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch("/api/convert/pdf-to-word", {
        ...
        body: JSON.stringify({ fileKey: `uploads/${file.name}`, from: "pdf", to: "docx" }),
      });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Convert PDF files to editable Word documents (.docx).</p>
        </div>
        <Card>
          <CardContent className="p-6 space-y-6">
            <FileDropzone onFilesSelect={(f) => setFile(f[0] || null)} accept={[".pdf"]} multiple={false} loading={loading} />
            {error && <div className="p-3 text-sm text-red-600 bg-red-50 rounded-lg">{error}</div>}
            {result && (
              <div className="p-4 bg-green-50 rounded-lg">
                <p className="text-green-700 mb-3">Conversion complete!</p>
                <Button asChild><a href={result} download><Download className="h-4 w-4 mr-2" /> Download Word</a></Button>
              </div>
            )}
            <Button onClick={handleConvert} disabled={!file || loading} className="w-full">
              {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Converting...</> : <><FileText className="h-4 w-4 mr-2" /> Convert to Word</>}
            </Button>
          </CardContent>
        </Card>
      </div>
